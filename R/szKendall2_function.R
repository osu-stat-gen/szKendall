...
#' @return A square szKendall2 dissimilarity matrix, where the dimension is the number of single cells.
#' @examples
#' foreach::registerDoSEQ()
#' szKendall2.diss(sim1.data, true1.data)
#' @export
szKendall2.diss <- function(sim.data, true.data){
  ...
  weight_sz_vec2 <- cal_weight_sz_vec2(n=n1)

  szkendall2.dist <- matrix(0, nrow=n.cells, ncol=n.cells)

  fail.index <- c()
  k <- 0
  for(i in 1:(n.cells-1)){
    for(j in (i+1):(n.cells)){
      if(szkendall2.dist[i,j]<1e-5){
        k <- k+1
  ...
  score2 <- foreach(para=1:nrow(fail.index), .combine = 'rbind', .noexport="szkendall2") %dopar%{
    value <- szkendall2(sim.data[,fail.index[para,1]], sim.data[,fail.index[para,2]], which(true.data[,fail.index[para,1]]==0), which(true.data[,fail.index[para,2]]==0), weight_sz_vec2, type="Nodiag")
    c(fail.index[para,1], fail.index[para,2], value)
  }

  for(m in 1:nrow(fail.index)){
    szkendall2.dist[score2[m,1], score2[m,2]] <- score2[m,3]
  }
  ...
  if(!is.null(dim(fail.index))) {
    warning("Parallel computing of szKendall2 among the cell pairs is not done properly. Please re-run the function and/or re-set the parallel backend.")
  }

  szkendall2.dist <- (szkendall2.dist + t(szkendall2.dist))

  rownames(szkendall2.dist) <- NULL
  colnames(szkendall2.dist) <- NULL

  return(szkendall2.dist)
}
